...
public class UrbanizacionPDF {
	
	private List<Urbanizacion> listas;

	public UrbanizacionPDF(List<Urbanizacion> listas) {
		super();
		this.listas = listas;
	}
	...
	private void escribirDatosDeLaTabla(PdfPTable tabla) {
		for (Urbanizacion list : listas) {
			tabla.addCell(String.valueOf(list.getId_urbanizacion()));
			tabla.addCell(list.getUrbanizacion());
			if(list.isEstado()) {
	...
	public void exportar(HttpServletResponse response) throws DocumentException, IOException {
		...
		Paragraph titulo = new Paragraph("LISTADO DE URBANIZACIONES", fuente);
